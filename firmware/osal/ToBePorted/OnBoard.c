 ...
 *
 *********************************************************************/
void _itoa(uint16_t num, uint8_t *buf, uint8_t radix)
{
  char c,i;
  uint8_t *p, rst[5];
...
// *********************************************************************/
//uint8_t RegisterForKeys( uint8_t task_id )
//{
...
//      msgPtr->keys = keys;
//
//      osal_msg_send( registeredKeysTaskID, (uint8_t *)msgPtr );
//    }
//    return ( SUCCESS );
//  }
//  else
...
//  (void)state;
//
//  // shift key (S6) is used to generate key interrupt
//  // applications should not use S6 when key interrupt is enabled
//  shift = (OnboardKeyIntEnable == HAL_KEY_INTERRUPT_ENABLE) ? false : ((keys & HAL_KEY_SW_0) ? true : false);
//
//  if ( OnBoard_SendKeys( keys, shift ) != SUCCESS )
//  {
//    if ( keys & HAL_KEY_SW_0 )  // Switch 1
//    {
//    }
//    // Process SW1 here
